...
This module is shaped as a self-contained library, not as a bridge
HTTP service: it has no aiohttp / web imports, no awareness of routes
or request objects, and no dependency on ``bridge.adapters``. Any
in-process consumer can import it and call :func:`search_all`. The
plugin's HTTP shim (``bridge/features/lark/routes.py``) is one such
consumer; future consumers (e.g. another Hermes plugin, the TUI's
``@``-completion path) could call the same surface unchanged.

Why this exists
---------------
The browser extension has no Feishu auth — it can't list the user's
groups / contacts / docs to populate a picker. ``lark-cli`` (already
on the user's machine, used by the Hermes lark skills) can. We shell
out to it across three entity types:
...
Future extraction
-----------------
If this module ever grows a second consumer outside the browser-
extension plugin, it can be lifted into a sibling Python package
(``hermes_lark/``) without code changes — its only external touchpoint
is the ``lark-cli`` binary on PATH and the ``HERMES_HOME`` environment
variable.
"""
...
def _hermes_home() -> Path:
    """Resolve ``HERMES_HOME`` without depending on bridge adapters.
